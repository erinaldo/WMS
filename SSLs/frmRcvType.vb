...
Public Class frmRcvType

    Public Stat As Boolean
    Public SId As Integer
    Dim codeOld As String

    Private Sub frmRcvType_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SelData()
    ...
    Sub SelData()
        Try
            ...
            Dim dtGrid = (From c In stRcvType.Instance.RcvType Order By c.CreateDate Select New With {c.Id, .รหัส = c.Code, .ประเภท = c.Name, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
            With DataGridView1
                .DataSource = dtGrid
    ...
    Private Sub DataGridView1_CellClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        If e.RowIndex <> -1 Then
            With DataGridView1
                ...
                Dim ds = (From c In stRcvType.Instance.RcvType Where c.Id = SId).FirstOrDefault
        ...
        If MessageBox.Show("ต้องการบันทึกข้อมูล ใช่หรือไม่?", "ยืนยันการบันทึก", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
            Try
                Using db = New PTGwmsEntities
                    If Stat = False Then
                        If Not IsNothing((From c In stRcvType.Instance.RcvType Where c.Code = tbCode.Text).FirstOrDefault) Then
                            MessageBox.Show("ป้อนรหัสซ้ำ", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            tbCode.Focus()
                            tbCode.SelectAll()
                            Exit Sub
                        End If
                        db.RcvType.Add(New RcvType With {.FKCompany = CompID, .Code = tbCode.Text, .Name = tbName.Text, .Description = tbDesc.Text, .CreateDate = DateTimeServer(), .CreateBy = Username, .UpdateDate = DateTimeServer(), .UpdateBy = Username, .Enable = True})
                        db.SaveChanges()
                        Dim lastadd = (From c In db.RcvType Order By c.UpdateDate Descending Where c.UpdateBy = Username).FirstOrDefault
                        stRcvType.Instance.RcvType.Add(lastadd)
                    Else
                        If tbCode.Text.Trim <> codeOld Then
                            ...
                            End If
                        End If
                        Dim ds = (From c In db.RcvType Where c.Id = SId).FirstOrDefault
                        If Not IsNothing(ds) Then
                            ds.FKCompany = CompID
                            ds.Code = tbCode.Text
                            ds.Name = tbName.Text
                            ...
                            db.SaveChanges()
                        End If
                        Dim ds1 = (From c In stRcvType.Instance.RcvType Where c.Id = SId).FirstOrDefault
                        If Not IsNothing(ds1) Then
                            stRcvType.Instance.RcvType.Remove(ds1)
                            Dim lastadd = (From c In db.RcvType Order By c.UpdateDate Descending Where c.UpdateBy = Username).FirstOrDefault
                            stRcvType.Instance.RcvType.Add(lastadd)
                        End If
                    End If
    ...
    Private Sub btExcel_Click(sender As System.Object, e As System.EventArgs) Handles btExcel.Click
        ...
        xlApp.ActiveSheet.Cells(1, 1) = "ข้อมูล ประเภทการรับสินค้า (" & Comp & ")"
