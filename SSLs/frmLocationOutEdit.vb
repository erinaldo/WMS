Public Class frmLocationOutEdit

    Public SId, ZId, LtId, WhID As Integer
    Dim codeOld As String

    Private Sub frmLocationOutEdit_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        codeOld = ""
        If sEdit = True Then
            Dim Sel = (From c In stLocation.Instance.Location Where c.Id = frmLocationOut.SId).FirstOrDefault
    ...
    Private Sub btDelete_Click(sender As System.Object, e As System.EventArgs) Handles btDelete.Click
        If MessageBox.Show("ต้องการลบข้อมูล ใช่หรือไม่?", "ยืนยันการลบข้อมูล", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
            Using db = New PTGwmsEntities
                Try
                    Dim ds = (From c In stLocation.Instance.Location Where c.Id = frmLocationOut.SId).FirstOrDefault
                    ...
                    End If
                    Dim ds1 = (From c In db.Locations Where c.Id = frmLocationOut.SId).FirstOrDefault
