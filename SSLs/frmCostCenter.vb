...
Public Class frmCostCenter
    ...
    Sub SelData()
        Try
            ...
            Dim dtGrid = (From c In stCostCenter.Instance.CostCnt Order By c.Code Select New With {c.Id, .รหัส = c.Code, .ชื่อ = c.Name, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
            With DataGridView1
                .DataSource = dtGrid
    ...
    Private Sub DataGridView1_CellClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        If e.RowIndex <> -1 Then
            With DataGridView1
                ...
                Dim ds = (From c In stCostCenter.Instance.CostCnt Where c.Id = SId).FirstOrDefault
    ...
    Private Sub btSave_Click(sender As Object, e As EventArgs) Handles btSave.Click
        ...
        End If
        If tbName.Text = "" Then
            MessageBox.Show("กรุณาป้อน ชื่อ", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Error)
            tbName.Focus()
            Exit Sub
        End If
        ...
        If MessageBox.Show("ต้องการบันทึกข้อมูล ใช่หรือไม่?", "ยืนยันการบันทึก", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
            Try
                Using db = New PTGwmsEntities
                    If Stat = False Then
                        If Not IsNothing((From c In stCostCenter.Instance.CostCnt Where c.Code = tbCode.Text).FirstOrDefault) Then
                            MessageBox.Show("ป้อนรหัสซ้ำ", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            tbCode.Focus()
                            tbCode.SelectAll()
                            Exit Sub
                        End If
                        db.CostCenter.Add(New CostCenter With {.FKCompany = CompID, .Code = tbCode.Text, .Name = tbName.Text, .Description = tbDesc.Text, .CreateDate = DateTimeServer(), .CreateBy = Username, .UpdateDate = DateTimeServer(), .UpdateBy = Username, .Enable = True})
                        db.SaveChanges()
                        btNew_Click(sender, e)
                        Dim lastadd = (From c In db.CostCenter Order By c.UpdateDate Descending Where c.FKCompany = CompID And c.UpdateBy = Username).FirstOrDefault
                        stCostCenter.Instance.CostCnt.Add(lastadd)
                    Else
                        If tbCode.Text <> codeOld Then
                            ...
                            End If
                        End If
                        Dim ds = (From c In db.CostCenter Where c.Id = SId).FirstOrDefault
                        If Not IsNothing(ds) Then
                            ds.FKCompany = CompID
                            ds.Code = tbCode.Text
                            ds.Name = tbName.Text
                            ...
                            db.SaveChanges()
                        End If
                        Dim ds1 = (From c In stCostCenter.Instance.CostCnt Where c.Id = SId).FirstOrDefault
                        If Not IsNothing(ds1) Then
                            stCostCenter.Instance.CostCnt.Remove(ds1)
                            Dim lastadd = (From c In db.CostCenter Order By c.UpdateDate Descending Where c.FKCompany = CompID And c.UpdateBy = Username).FirstOrDefault
                            stCostCenter.Instance.CostCnt.Add(lastadd)
                        End If
                    End If
    ...
    Private Sub tbSearch_TextChanged(sender As System.Object, e As System.EventArgs) Handles tbSearch.TextChanged
        Try
            Dim dtGrid = (From c In stCostCenter.Instance.CostCnt Order By c.CreateDate Where c.Code.Contains(tbSearch.Text) Or c.Name.Contains(tbSearch.Text) Or c.Description.Contains(tbSearch.Text) Select New With {c.Id, .รหัส = c.Code, .ชื่อ = c.Name, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
    ...
    Private Sub btExcel_Click(sender As System.Object, e As System.EventArgs) Handles btExcel.Click
        ...
        xlApp.ActiveSheet.Cells(1, 1) = "ข้อมูล CostCenter"
