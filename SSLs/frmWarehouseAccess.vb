...
Public Class frmWarehouseAccess

    Public dt As New List(Of WarehouseAccess)
    Public Stat As Boolean
    Public SId, WhID As Integer
    Private Sub frmWarehouseAccess_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.Text = "สิทธิ์การเข้าถึงคลังสินค้า ของผู้ใช้ : " & frmUser.tbName.Text
        SelData()
    ...
    Sub SelData()
        Try
            Using db = New PTGwmsEntities
                dt = db.WarehouseAccess.Include("Warehouse").Where(Function(x) x.Enable = True And x.FKCompany = CompID And x.FKUser = frmUser.tbUserId.Text).ToList
            End Using
            Dim dtGrid = (From c In dt Order By c.Id Select New With {c.Id, c.FKWarehouse, .WHCode = c.Warehouse.Code, .คลังสินค้า = c.Warehouse.Name, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
            With DataGridView1
                .DataSource = dtGrid
    ...
    Private Sub btSave_Click(sender As Object, e As EventArgs) Handles btSave.Click
        ...
        End If
        If Stat = False Then
            If Not IsNothing((From c In dt Where c.Warehouse.Code = tbWh.Text).FirstOrDefault) Then
        ...
        If MessageBox.Show("ต้องการบันทึกข้อมูล ใช่หรือไม่?", "ยืนยันการบันทึก", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
            Try
                Using db = New PTGwmsEntities
                    If Stat = False Then
                        db.WarehouseAccess.Add(New WarehouseAccess With {.FKCompany = CompID, .FKUser = frmUser.tbUserId.Text, .FKWarehouse = WhID, .Description = tbDesc.Text, .CreateDate = DateTimeServer(), .CreateBy = Username, .UpdateDate = DateTimeServer(), .UpdateBy = Username, .Enable = True})
                    Else
                        Dim ds = (From c In db.WarehouseAccess Where c.Id = SId).FirstOrDefault
    ...
    Private Sub tbSearch_TextChanged(sender As System.Object, e As System.EventArgs) Handles tbSearch.TextChanged
        Try
            Dim dtGrid = (From c In dt Order By c.Id Where c.Warehouse.Code.Contains(tbSearch.Text) Or c.Warehouse.Name.Contains(tbSearch.Text) Or c.Description.Contains(tbSearch.Text) Select New With {c.Id, c.FKWarehouse, .WhCode = c.Warehouse.Code, .คลังสินค้า = c.Warehouse.Name, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
    ...
    Private Sub btExcel_Click(sender As System.Object, e As System.EventArgs) Handles btExcel.Click
        ...
        xlApp.ActiveSheet.Cells(1, 1) = "ข้อมูล การเข้าถึงคลังสินค้า ของผู้ใช้ : " & frmUser.tbName.Text
