...
Public Class frmPickingType

    Public Stat As Boolean
    Public SId As Integer
    Dim codeOld As String

    Private Sub frmPickingType_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SelData()
    ...
    Private Sub TextBox_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles tbCode.KeyDown, tbName.KeyDown, tbDesc.KeyDown, tbAccID.KeyDown
    ...
    Sub SelData()
        Try
            ...
            Dim dtGrid = (From c In stPickType.Instance.PickType Order By c.Code Select New With {c.Id, .รหัส = c.Code, .ประเภท = c.Name, .AccountCode = c.ACCCODE, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
            With DataGridView1
                .DataSource = dtGrid
    ...
    Private Sub DataGridView1_CellClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        If e.RowIndex <> -1 Then
            With DataGridView1
                ...
                Dim ds = (From c In stPickType.Instance.PickType Where c.Id = SId).FirstOrDefault
                If Not IsNothing(ds) Then
                    tbCode.Text = ds.Code
                    codeOld = ds.Code
                    tbName.Text = ds.Name
                    tbAccID.Text = ds.ACCCODE
        ...
        If MessageBox.Show("ต้องการบันทึกข้อมูล ใช่หรือไม่?", "ยืนยันการบันทึก", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = DialogResult.OK Then
            Try
                Using db = New PTGwmsEntities
                    If Stat = False Then
                        If Not IsNothing((From c In stPickType.Instance.PickType Where c.Code = tbCode.Text).FirstOrDefault) Then
                            MessageBox.Show("ป้อนรหัสซ้ำ", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            tbCode.Focus()
                            tbCode.SelectAll()
                            Exit Sub
                        End If
                        db.PickingType.Add(New PickingType With {.FKCompany = CompID, .Code = tbCode.Text, .Name = tbName.Text, .ACCCODE = tbAccID.Text, .Description = tbDesc.Text, .CreateDate = DateTimeServer(), .CreateBy = Username, .UpdateDate = DateTimeServer(), .UpdateBy = Username, .Enable = True})
                        db.SaveChanges()
                        btNew_Click(sender, e)
                        Dim lastadd = (From c In db.PickingType Order By c.UpdateDate Descending Where c.FKCompany = CompID And c.UpdateBy = Username).FirstOrDefault
                        stPickType.Instance.PickType.Add(lastadd)
                    Else
                        If tbCode.Text <> codeOld Then
                            ...
                            End If
                        End If
                        Dim ds = (From c In db.PickingType Where c.Id = SId).FirstOrDefault
                        If Not IsNothing(ds) Then
                            ds.FKCompany = CompID
                            ds.Code = tbCode.Text
                            ds.Name = tbName.Text
                            ds.ACCCODE = tbAccID.Text
                            ds.Description = tbDesc.Text
                            ds.UpdateDate = DateTimeServer()
                            ds.UpdateBy = Username
                            db.SaveChanges()
                        End If
                        Dim ds1 = (From c In stPickType.Instance.PickType Where c.Id = SId).FirstOrDefault
                        If Not IsNothing(ds1) Then
                            stPickType.Instance.PickType.Remove(ds1)
                            Dim lastadd = (From c In db.PickingType Order By c.UpdateDate Descending Where c.FKCompany = CompID And c.UpdateBy = Username).FirstOrDefault
                            stPickType.Instance.PickType.Add(lastadd)
                        End If
                    End If
    ...
    Private Sub tbSearch_TextChanged(sender As System.Object, e As System.EventArgs) Handles tbSearch.TextChanged
        Try
            Dim dtGrid = (From c In stPickType.Instance.PickType Order By c.CreateDate Where c.Code.Contains(tbSearch.Text) Or c.Name.Contains(tbSearch.Text) Or c.ACCCODE.Contains(tbSearch.Text) Or c.Description.Contains(tbSearch.Text) Select New With {c.Id, .รหัส = c.Code, .ประเภท = c.Name, .AccountCode = c.ACCCODE, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
    ...
    Private Sub btExcel_Click(sender As System.Object, e As System.EventArgs) Handles btExcel.Click
        ...
        xlApp.ActiveSheet.Cells(1, 1) = "ข้อมูล ประเภทการเบิกสินค้า"
