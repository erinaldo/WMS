...
Public Class frmLocationOut

    Public SId As Integer

    Private Sub frmLocationOut_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SelData()
    End Sub
    ...
    Sub SelData()
        Try
            If RoleID = 1 Then
                ...
                End With
            Else
                Dim dtGrid = (From c In stLocation.Instance.Location Order By c.Warehouse.Code, c.Name Where c.InStorage = False And frmMain.wh.Contains(c.Warehouse.Code.ToString) Select New With {c.Id, .WHCode = c.Warehouse.Code, .คลังสินค้า = c.Warehouse.Name, .Location = c.Name, .ZoneCode = c.Zone.Code, .โซน = c.Zone.Name, .TypeCode = c.LocationType.Code, .ประเภท = c.LocationType.Name, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
                With DataGridView1
                    .DataSource = dtGrid
                    .Columns("Id").Visible = False
                    .AutoResizeColumns()
    ...
    Private Sub tbSearch_TextChanged(sender As System.Object, e As System.EventArgs) Handles tbSearch.TextChanged
        Try
            If RoleID = 1 Then
                Dim dtGrid = (From c In stLocation.Instance.Location Order By c.Warehouse.Code, c.Name Where c.InStorage = False And (c.Warehouse.Code.Contains(tbSearch.Text) Or c.Warehouse.Name.Contains(tbSearch.Text) Or c.Name.Contains(tbSearch.Text) Or c.Zone.Code.Contains(tbSearch.Text) Or c.Zone.Name.Contains(tbSearch.Text) Or c.LocationType.Code.Contains(tbSearch.Text) Or c.LocationType.Name.Contains(tbSearch.Text) Or c.Description.Contains(tbSearch.Text)) Select New With {c.Id, .WHCode = c.Warehouse.Code, .คลังสินค้า = c.Warehouse.Name, .Location = c.Name, .ZoneCode = c.Zone.Code, .โซน = c.Zone.Name, .TypeCode = c.LocationType.Code, .ประเภท = c.LocationType.Name, .หมายเหตุ = c.Description, .CreateDate = Format(c.CreateDate, "dd/MM/yyyy HH:mm"), .CreateBy = c.CreateBy, .UpdateDate = Format(c.UpdateDate, "dd/MM/yyyy HH:mm"), .UpdateBy = c.UpdateBy}).ToList
    ...
    Private Sub btExcel_Click(sender As System.Object, e As System.EventArgs) Handles btExcel.Click
        ...
        xlApp.ActiveSheet.Cells(1, 1) = "ข้อมูล Location นอกพื้นที่จัดเก็บ (" & Comp & ")"
